...
%bcond_with provides_community_mysql
# Obsolete the package 'community-mysql' and all its sub-packages
%bcond_with obsoletes_community_mysql
...
BuildRequires:    gcc-toolset-14-gcc gcc-toolset-14-gcc-c++ gcc-toolset-14-binutils gcc-toolset-14-libstdc++-devel gcc-toolset-14-annobin-plugin-gcc gcc-toolset-14-annobin-annocheck 
...
install -p -m 0755 %{_builddir}/%{name}-%{version}/scripts/mysql_config_multilib %{buildroot}%{_bindir}/mysql_config
fi

# install INFO_SRC, INFO_BIN into libdir (upstream thinks these are doc files,
# but that's pretty wacko --- see also %%{name}-file-contents.patch)
install -p -m 0644 %{_builddir}/%{name}-%{version}/Docs/INFO_SRC %{buildroot}%{_libdir}/mysql/
install -p -m 0644 %{_builddir}/%{name}-%{version}/Docs/INFO_BIN %{buildroot}%{_libdir}/mysql/
...
%if %{with config}
install -D -p -m 0644 %{_builddir}/%{name}-%{version}/scripts/my.cnf %{buildroot}%{_sysconfdir}/my.cnf
%endif

# install systemd unit files and scripts for handling server startup
install -D -p -m 644 %{_builddir}/%{name}-%{version}/scripts/mysql.service %{buildroot}%{_unitdir}/%{daemon_name}.service
install -D -p -m 644 %{_builddir}/%{name}-%{version}/scripts/mysql@.service %{buildroot}%{_unitdir}/%{daemon_name}@.service
install -D -p -m 0644 %{_builddir}/%{name}-%{version}/scripts/mysql.tmpfiles.d %{buildroot}%{_tmpfilesdir}/%{daemon_name}.conf
rm -r %{buildroot}%{_tmpfilesdir}/mysql.conf

# helper scripts for service starting
install -D -p -m 755 %{_builddir}/%{name}-%{version}/scripts/mysql-prepare-db-dir %{buildroot}%{_libexecdir}/mysql-prepare-db-dir
install -p -m 755 %{_builddir}/%{name}-%{version}/scripts/mysql-wait-stop %{buildroot}%{_libexecdir}/mysql-wait-stop
install -p -m 755 %{_builddir}/%{name}-%{version}/scripts/mysql-check-socket %{buildroot}%{_libexecdir}/mysql-check-socket
install -p -m 644 %{_builddir}/%{name}-%{version}/scripts/mysql-scripts-common %{buildroot}%{_libexecdir}/mysql-scripts-common
install -D -p -m 0644 %{_builddir}/%{name}-%{version}/scripts/server.cnf %{buildroot}%{_sysconfdir}/my.cnf.d/%{majorname}-server.cnf
...
install -D -m 0644 %{_builddir}/%{name}-%{version}/packaging/rpm-common/mysql.logrotate %{buildroot}%{logrotateddir}/%{daemon_name}
